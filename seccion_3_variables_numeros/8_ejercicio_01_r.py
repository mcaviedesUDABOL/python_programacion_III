## Crear una variable "numero1" que tenga el valor de 5
# Indicar de que tipo es la variable "numero1"

numero1 = 5
print(numero1)
print(type(numero1))

# Crear otra varivale "numero2" que tenga el valor de 6.5
# indicar de que tipo es la variable "numero2"

numero2 = 6.5
print(numero2)
print(type(numero2))

# Crear otra varivale "numero3" que tenga el valor de 7
# indicar de que tipo es la variable "numero3"

numero3 = 7
print(numero3)
print(type(numero3))

# Crear otra variable "suma" que tenga el valor de la suma de los 3 numeros anteriores (numero1, numero2, numero3)
# Mostrar el resultado de la variable "suma"
# Indicar de que tipo es la variable suma

suma = numero1 + numero2 + numero3
print(suma)
print(type(suma))



